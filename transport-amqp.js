...
function AmqpTransport(options, _, amqplib, Promise, serializer, uuid) {
  ...
  function bindQueue(descriptor) {
    ...
    var bindInfo = {
      queue: descriptor.ep.queue,
      exchange: descriptor.ep.exchange,
  ...
  function debug(label, message) {
    ...
    var text = '[AmqpTransport.' + Array.prototype.shift.call(arguments) + '] ' + Array.prototype.shift.call(arguments),
      argumentsUsed = [],
      match, re = /\{\d+\}/gm;
  ...
  function declareExchange(descriptor) {
    ...
    exchangeInfo = {
      name: descriptor.ep.exchange,
      type: descriptor.ep.exchangeType,
      options: {
        durable: false
      }
    };
    ...
    return function receive(x) {
      return Promise
        .try(function () {
          ...
          var matches = _.filter(descriptors, function (d) {
            return d.callback && d.matches(mc);
          });
          if (matches.length < 1) {
            throw _.extend(new Error(util.format('Unhandled message:', mc)), {
              mc: mc
            });
          }
